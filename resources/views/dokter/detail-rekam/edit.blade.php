...
<div class="container-fluid">
    ...
    <div class="card border-0 shadow-sm">
        <div class="card-header bg-white border-bottom">
            <h5 class="mb-0"><i class="fas fa-edit text-warning"></i> Edit Detail Rekam Medis</h5>
        </div>
        <div class="card-body">
            <form action="{{ route('dokter.detail-rekam.update', $detail->iddetail_rekam_medis) }}" method="POST">
                @csrf
                @method('PUT')
                    ...
                    <select name="idrekam_medis" class="form-select @error('idrekam_medis') is-invalid @enderror" required>
                        ...
                        @foreach($rekamMedis as $rekam)
                            <option value="{{ $rekam->idrekam_medis }}" 
                                {{ old('idrekam_medis', $detail->idrekam_medis) == $rekam->idrekam_medis ? 'selected' : '' }}>
                ...
                <div class="mb-3">
                    ...
                    <textarea name="detail" 
                              ...
                              rows="4" 
                              placeholder="Masukkan detail atau catatan tambahan...">{{ old('detail', $detail->detail) }}</textarea>
                    @error('detail')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
