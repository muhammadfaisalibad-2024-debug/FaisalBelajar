...
@section('title', 'Edit Hewan Peliharaan - RSHP')

@section('content')
<div class="row">
    <div class="col-12">
        <div class="d-flex justify-content-between align-items-center mb-4">
            <h2><i class="bi bi-pencil-square"></i> Edit Hewan Peliharaan</h2>
        ...
        <div class="card shadow-sm">
            <div class="card-body">
                <form action="{{ route('pets.update', $pet) }}" method="POST" enctype="multipart/form-data">
                            ...
                            <select class="form-select @error('animal_type_id') is-invalid @enderror" 
                                ...
                                @foreach($animalTypes as $type)
                                    <option value="{{ $type->id }}" 
                                            {{ old('animal_type_id', $pet->animal_type_id) == $type->id ? 'selected' : '' }}>
                                        {{ $type->name }}
                                    </option>
                                @endforeach
                            </select>
                            ...
                            <select class="form-select @error('animal_breed_id') is-invalid @enderror" 
                                ...
                                @foreach($animalBreeds as $breed)
                                    <option value="{{ $breed->id }}" 
                                            {{ old('animal_breed_id', $pet->animal_breed_id) == $breed->id ? 'selected' : '' }}>
                                        {{ $breed->name }}
                                    </option>
                                @endforeach
                            </select>
                    ...
                    <div class="mb-3">
                        <label for="photo" class="form-label">{{ $pet->photo ? 'Ganti Foto' : 'Foto Hewan' }}</label>
                        <input type="file" 
                    ...
                    <div class="mb-3">
                        ...
                        <textarea class="form-control @error('special_notes') is-invalid @enderror" 
                                  ...
                                  name="special_notes" 
                                  rows="3"
                                  placeholder="Contoh: Alergi makanan tertentu, penyakit bawaan, dll">{{ old('special_notes', $pet->special_notes) }}</textarea>
                        @error('special_notes')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
